...
abstract public class AbstractHttpApi implements HttpApi {
    ...
    public FoursquareType executeHttpRequest(HttpRequestBase httpRequest,
        ...
        if (DEBUG) Log.i(PartyBolle.LOG_TAG, "doHttpRequest: " + httpRequest.getURI());

        HttpResponse response = executeHttpRequest(httpRequest);
        if (DEBUG) Log.i(PartyBolle.LOG_TAG, "executed HttpRequest for: "
        ...
        switch (response.getStatusLine().getStatusCode()) {
            ...
            case 401:
                ...
                if (DEBUG) Log.i(PartyBolle.LOG_TAG, "HTTP Code: 401");
            ...
            case 500:
                response.getEntity().consumeContent();
                if (DEBUG) Log.i(PartyBolle.LOG_TAG, "HTTP Code: 500");
                throw new FoursquareException("Foursquare is down. Try again later.");

            default:
                if (DEBUG) Log.i(PartyBolle.LOG_TAG, "Default case for status code reached: "
    ...
    public String doHttpPost(String url, NameValuePair... nameValuePairs)
            ...
            IOException {
        if (DEBUG) Log.i(PartyBolle.LOG_TAG, "doHttpPost: " + url);
        HttpPost httpPost = createHttpPost(url, nameValuePairs);

        HttpResponse response = executeHttpRequest(httpPost);
        if (DEBUG) Log.i(PartyBolle.LOG_TAG, "executed HttpRequest for: " + httpPost.getURI().toString());
    ...
    public HttpResponse executeHttpRequest(HttpRequestBase httpRequest) throws IOException {
        if (DEBUG) Log.i(PartyBolle.LOG_TAG, "executing HttpRequest for: "
                + httpRequest.getURI().toString());
        try {
    ...
    public HttpGet createHttpGet(String url, NameValuePair... nameValuePairs) {
        if (DEBUG) Log.i(PartyBolle.LOG_TAG, "creating HttpGet for: " + url);
        ...
        if (DEBUG) Log.i(PartyBolle.LOG_TAG, "Created: " + httpGet.getURI());
        return httpGet;
    }

    public HttpPost createHttpPost(String url, NameValuePair... nameValuePairs) {
        if (DEBUG) Log.i(PartyBolle.LOG_TAG, "creating HttpPost for: " + url);
        ...
        if (DEBUG) Log.i(PartyBolle.LOG_TAG, "Created: " + httpPost);
        return httpPost;
    }
    ...
    private List<NameValuePair> stripNulls(NameValuePair... nameValuePairs) {
        ...
        for (int i = 0; i < nameValuePairs.length; i++) {
            ...
            if (param.getValue() != null) {
                if (DEBUG) Log.i(PartyBolle.LOG_TAG, "Param: " + param);
                params.add(param);
            }
        }
        return params;
